...
import {
  ...
  useSetBlockMutation,
  useCreateBlockWithTimesMutation,
  useMeQuery,
...
const Index = () => {
  ...
  const [creatBlockWithTimes, createBlockWithTimesMutation] =
    useCreateBlockWithTimesMutation();
  ...
  const getNormalizedGCalDateEvent = (date) => {
    ...
    axios
      .post(
        "http://localhost:4000/api/get-daily-events",
        { date, refresh_token_data },
        ...
        if (resp.data) {
          ...
          if (valArr) {
            ...
            valArr.forEach((overlyingevent: string[][]) => {
              ...
              const start_as_json: string = Object.values(
                overlyingevent[10][1]
              )[0];
